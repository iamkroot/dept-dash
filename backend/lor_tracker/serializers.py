...
class GetFacultyListSerializer(serializers.ModelSerializer):
    class Meta:
        model = Faculty
        fields = ['psrn', 'email', 'name']


class GetStudentSerializer(serializers.ModelSerializer):
    class Meta:
        model = Student
        fields = ['id_num', 'email', 'name']
...
class CreateProfileSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        entry = StudentDetails.objects.create(
            ...
            user=Student.objects.get(id_num=self.context['request'].user.id_num)
        )
        return entry
...
class EditLorRequestSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        if validated_data.get('deadline', instance.deadline).replace(tzinfo=None) <= datetime.now().replace(
                tzinfo=None):
            errors = {
                "deadline": 'Invalid deadline'}
            raise ValidationError(errors)
